"""
M1 两阶段分布式优化器 (Naive Allocation Version)

核心策略：
  1. 均匀分配：轮询分配导弹给UAV（完全不考虑空间距离！）
  2. Phase 1: 分布式独立优化 - M个子任务，每对(UAV, PseudoCluster)独立优化
  3. Phase 2: 基因拼接与微调 - 拼接成50维超级个体

M1 特点：
  - 使用 NaiveAllocator（轮询分配）
  - 使用 PseudoCluster（与 MissileCluster 接口100%对齐）
  - 先验"撕裂"：质心可能在"无人区"，时间窗极宽
"""
...
def _evaluate_multi_missiles(args):
    ...
    Args:
        args: (params, uav_num, missile_configs, safe_distance)
            ...
            - uav_num: UAV编号
            - missile_configs: 导弹配置列表（一个PseudoCluster内的所有导弹）
...
class SubTaskGA:
    """
    单UAV-多导弹子任务GA优化器（10维空间）- M1版本

    优化目标：最大化对PseudoCluster内所有导弹的拦截成功率
    满分 = PseudoCluster内导弹数量 × 10000
    """

    def __init__(self, uav_num: int, cluster: PseudoCluster,
                 expert_gene: Dict, pop_size: int = 20, max_generations: int = 20,
                 n_threads: int = None):
        """
        Args:
            uav_num: UAV编号
            cluster: 伪导弹簇（PseudoCluster，与MissileCluster接口一致）
    ...
    def _init_population(self) -> List[List]:
        ...
        for i in range(self.pop_size):
            ...
            if i < n_expert:
                ...
                # 中点附近密集投放（M1特点：撕裂的时间窗）
                t_min, t_max = self.expert_gene["time_window"]
                t_mid = (t_min + t_max) / 2.0
                offsets = [-2.0, 0.0, 2.0]  # 密集投放
    ...
    def optimize(self) -> Tuple[List, float, int]:
        ...
        print(f"\n[子任务 M1] UAV{self.uav_num} vs PseudoCluster({self.n_missiles}导弹: {missile_ids}) 开始优化...")
        ...
        # 早停参数
        _PATIENCE  = 30       # 连续多少代无进展则放弃（增大以给GA更多时间找到窄有效时间窗）
...
class TwoStageOptimizer:
    ...
    Phase 1: M个子任务独立优化（10维空间，M = PseudoCluster数量）
    Phase 2: 基因拼接与全局微调（50维空间）

    关键特点：使用 NaiveAllocator 进行轮询分配
    """
    ...
    def __init__(self, uav_group: List[int], missile_configs: List[Dict],
        ...
        # 创建均匀分配器（M1核心特点！）
        self.allocator = NaiveAllocator(uav_group, missile_configs)

    def run_phase1(self) -> Dict[int, Tuple[List, float, int]]:
        """Phase 1: 分布式独立优化"""
        print("\n" + "=" * 70)
        print("  PHASE 1 M1: 分布式独立优化 (子任务 × 10维空间)")
        print("  策略: 均匀分配 -> PseudoCluster -> 分布式优化")
        print("=" * 70)
        ...
        for uav_id in self.uav_group:
            expert_gene = self.allocator.get_expert_genes(uav_id)
            if expert_gene:
                expert_genes[uav_id] = expert_gene
                print(f"\n[专家先验] UAV{uav_id}:")
                print(f"  时间窗: [{expert_gene['time_window'][0]:.1f}s, {expert_gene['time_window'][1]:.1f}s]")
                print(f"  目标导弹: {expert_gene['n_missiles']}枚")

        # 串行执行子任务
        subtask_results = {}
        t0 = time.time()

        for uav_id in sorted(self.allocator.get_assignment().keys()):
            cluster = self.allocator.get_clusters()[self.allocator.get_assignment()[uav_id]]
