...
class SpatioTemporalAllocator:
        ...
        for i, m1 in enumerate(sorted_missiles):
            ...
            for j, m2 in enumerate(sorted_missiles):
                ...
                for cm in cluster_missiles:
                    ...
                    if (np.linalg.norm(cm_pos[:2] - pos2[:2]) < self.SPATIAL_THRESHOLD and
                        abs(cm_t - t2) < self.TEMPORAL_THRESHOLD):
                        can_merge = True
                        break
    ...
    def print_clusters(self):
        ...
        print("[时空聚类] 导弹簇划分结果")
        print("=" * 70)
        print(f"  聚类参数: 空间阈值={self.SPATIAL_THRESHOLD}m, 时间阈值={self.TEMPORAL_THRESHOLD}s")
...
if __name__ == "__main__":
    ...
    # 加载Wave 2导弹配置（10枚导弹）
    with open("data/Wave_2_10_Missiles.json", "r", encoding="utf-8") as f:
        data = json.load(f)

    missile_configs = data["missiles"]
    uav_group = [1, 2, 3, 4, 5]

    print("=" * 70)
    print("时空聚类分配器 V2 测试 - Wave 2 (10导弹)")
    print("=" * 70)

    # 创建分配器
    allocator = SpatioTemporalAllocator(uav_group, missile_configs)

    # 执行时空聚类
    clusters = allocator.cluster_missiles()
    allocator.print_clusters()

    # 构建代价矩阵
    cost_matrix = allocator.compute_cost_matrix()
    print("\n代价矩阵 (UAV x Cluster):")
    print(cost_matrix.round(0).astype(int))

    # 执行匈牙利匹配
    assignment = allocator.hungarian_assignment()
    allocator.print_assignment()

    # 生成专家先验基因
    expert_genes = allocator.generate_expert_genes()
    ...
    for uav_id, gene in sorted(expert_genes.items()):
        ...
        print(f"    方向: ({gene['direction'][0]:.3f}, {gene['direction'][1]:.3f}, {gene['direction'][2]:.3f})")
        print(f"    时间窗: [{gene['time_window'][0]:.1f}s, {gene['time_window'][1]:.1f}s]")
        print(f"    覆盖导弹到达时间: [{cluster.earliest_arrival:.1f}s, {cluster.latest_arrival:.1f}s]")
