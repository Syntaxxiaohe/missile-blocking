...
# ===== 第三步：导入本地模块（M3特有：13维基因+弹药惩罚）=====
from optimizer.distributed_optimizer import DistributedOptimizer_M2, run_distributed_optimization_M2
...
def run_distributed():
    ...
    # 运行分布式优化
    best_params, best_fitness, info = run_distributed_optimization_M2(
        uav_group=[1, 2, 3, 4, 5],
        missile_configs=missile_configs,
        pop_size=20,
        max_generations=20
...
def run_wave3_distributed(pop_size=30, max_gen=50, n_threads=None):
    ...
    """
    print("\n" + "=" * 70)
    print("  WAVE 3 战役 (M3_AmmoPenalty): 5 UAVs vs 20 Missiles")
    print("  策略: 时空聚类 -> 匈牙利匹配 -> 分布式优化 -> 基因拼接")
...
def _evaluate_single_run_m3(args):
    """
    M3 单次评估顶层函数（subprocess 安全）。

    Args:
        args: (eval_idx, missiles, pop_size, max_gen)

    Returns:
        结果字典：eval_idx, intercepted, best_fitness, n_missiles, n_clusters
    """
...
def run_multi_eval_m3(
    ...
    """
    M3 多轮并行评估（ProcessPoolExecutor）。

    M3 独特优势：smokes_used 会随拦截成功率变化（弹药惩罚驱动）。
    ...
    Returns:
        汇总结果字典（含 ammo_efficiency 效费比统计）
    """
    ...
    print(f"\n{'='*70}")
    print(f"  M3_AmmoPenalty | Multi-Eval: {pattern_name}")
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(_evaluate_single_run_m3, task): task[0]
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                results.append({
                    ...
                    "intercepted": 0, "n_missiles": 20,
                    "n_clusters": 0, "error": str(e),
                })
    ...
    print(f"\n{'='*70}")
    print(f"  SUMMARY: M3_OT (最优传输) | {pattern_name}")
    ...
    out_dir = os.path.join(ROOT_DIR, "M3_AmmoPenalty", "results")
    os.makedirs(out_dir, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_path = os.path.join(out_dir, f"M3_{pattern_name}_{ts}.json")
...
def main():
    parser = argparse.ArgumentParser(description="M3_AmmoPenalty 导弹雨防御大阵")
